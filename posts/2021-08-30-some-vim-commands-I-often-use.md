...
![vim: #](https://storage.googleapis.com/homuchen.com/images/vim-5.jpg)
...
![dw](https://storage.googleapis.com/homuchen.com/images/vim-6.jpg)

## d + {navigation key}
搭配上面提到的各種navigation的方法，比如說:

* `dw` : 刪掉一個字，從指標到字的尾端，像是下圖反白的部分。
  ![dw](https://storage.googleapis.com/homuchen.com/images/vim-1.jpg)
* `d%` : 刪掉整個(...)、[...]或{...}

## di

* `diw`：跟`dw`也是刪掉一個字，但是可以刪掉整個字，效果就相當於先按`b`讓指標回到字首再`dw`。
  ![diw](https://storage.googleapis.com/homuchen.com/images/vim-2.jpg)
* `di"` or `di(` or `di[` or `di{`：刪掉""內、()、[]及{}的所有字。

  `di(` or `di)`即可刪掉()中function的paramter
  ![di(](https://storage.googleapis.com/homuchen.com/images/vim-3.jpg)

  `di"`則是刪掉雙引號中間的所有字，留下雙引號
  ![di"](https://storage.googleapis.com/homuchen.com/images/vim-4.jpg)
