...
如果對於HTTP message format還不熟或想要有更多了解的，可以看我之前寫的文章: [**HTTP 簡介，以及使用telnet、nc、curl等指令來探索**](/posts/http-introduction-telnet-nc-curl/){:target="_blank"}
...
* **Content-Type**: 用來說明body裡的資料是什麼格式，常見的像是html檔案的`text/html`、csv檔案的`application/csv`、圖片可能是`image/jpg`
  、你的API可能用的是`application/json`等等，如果沒有Content-Type，收到資料的一方將不知道該要如何處理這些資料，
   至於要用哪種type可以在這查詢[MIME 類別](https://developer.mozilla.org/zh-TW/docs/Web/HTTP/Basics_of_HTTP/MIME_types){:target="_blank"}
...
相關的RFC: [RFC 7232 "HTTP: Conditional Requests](https://tools.ietf.org/html/rfc7232){:target="_blank"}。
...
* [HTTP headers](https://developer.mozilla.org/zh-TW/docs/Web/HTTP/Headers){:target="_blank"}
* [MIME 類別](https://developer.mozilla.org/zh-TW/docs/Web/HTTP/Basics_of_HTTP/MIME_types){:target="_blank"}
* [RFC 7232 "HTTP: Conditional Requests](https://tools.ietf.org/html/rfc7232){:target="_blank"}
