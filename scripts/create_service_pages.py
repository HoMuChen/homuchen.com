...
PAGES = [
    {
        ...
        "markdown": """
你有沒有算過，每天有多少時間，花在「其實不需要你親自做」的事情上？

整理資料、複製貼上、把東西從 A 系統搬到 B 系統、每天早上固定要查一輪的數據、寫那些格式都差不多的內容⋯⋯這些事不難，但就是吃時間。而且最煩的是——**它們每天都會回來找你。**

我做的事情很單純：**進到你的環境，幫你架一隻能自己把這些事做完的 AI Agent。** 不是「幫你生一段文字」的玩具，也不是再賣你一套要登入、要付月費的 SaaS——是一隻真正跑在**你自己的電腦、工作站或伺服器上**、串接你現有工具、每天自己開工的 agent。

## 這不是 demo，是真的在跑的東西

我自己電腦上就有好幾隻 agent 天天在跑：

### 內容行銷自動化

從 SEO 關鍵字研究 → 系列文章規劃 → 用我的寫作風格產出 → 自動建立內部連結 → 發布到 Ghost。原本寫一篇 SEO 友善的文章要花半天到一天，現在整條產線壓縮到只剩我做決策，連 SEO 結構的一致性都比人工更穩。

### 電商營運自動化

商品上下架、圖片更新、內容產出——電商日常最重複的那些動作交給 agent，人只負責做決策。

### 量化交易資訊收集

盤中盯盤、收盤補資料、偵測到訊號就傳到我手機。以前每天早上要花一兩小時才看得完的資訊，現在我只要看它整理好的結論。

> 想看這些是怎麼做出來的？我把完整觀念和做法寫成了 [**自架 AI Agent 實戰系列**](/posts/self-hosted-ai-agent/)。

## 跟「買一套工具」差在哪？

市面上的 AI 自動化，多半是叫你去學一套新平台、或租一套別人做好的 SaaS。但你的流程是你的，別人的模板套上來總是差一截，資料還得交到第三方手上。

我的做法不一樣：

- **進你的環境，不用搬家**——agent 跑在你自己的機器上，串你現在就在用的工具，資料不外流。
- **依你的需求和習慣選工具**——Claude Code、Codex、OpenClaw、Hermes⋯⋯沒有「最好」的工具，只有最適合你的。挑哪個、怎麼設定，是我的事。
- **諮詢、代建都可以**——你想自己動手，我幫你看怎麼設計最合理；你沒空，我直接幫你做到好。
- **個人到中小企業都接**——一人接案、小團隊、中小企業都歡迎（沒有要服務超級大公司啦😂）。
...
重點從來不是「這件事 AI 能不能做」，而是——**你願不願意把它從你的待辦清單上永遠刪掉。**

## 我怎麼幫你

1. **了解你的流程**——先聊你現在怎麼做，找出最值得自動化的環節
2. **選對工具、設計架構**——依你的需求和習慣，從 Claude Code、Codex、OpenClaw、Hermes 裡挑最適合的，設計它的指令、知識來源、工具串接和決策邏輯
3. **進你的環境架設**——直接在你自己的機器上把 agent 架起來，串進你現有的系統，不用搬家、資料也不外流
4. **持續調校**——上線後依實際使用情況優化它的表現

## 先聊聊你的狀況

不用一開始就想做大。先跟我說你每天最煩、最重複的那件事，我幫你看哪個環節最適合先動手——不一定要馬上做，先聊聊、評估一下都可以。

你只需要告訴我一句：**「這件事我不想再自己做了。」**
""",
    },
    {
        "title": "AI 客製化系統建置",
